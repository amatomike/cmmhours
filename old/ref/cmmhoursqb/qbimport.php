 ...
 *
 */
define('QB_PRIORITY_PURCHASEORDER', 3);
 ...
 */
define('QB_QUICKBOOKS_MAILTO', 'mike@mikeamato.org');
...
$dsn = 'mysql://root:easy2use@localhost/quickbooks_import';
...
{
    error_log('###################################################'.'got customer response ...    \n'.$xml);
    if (!empty($idents['iteratorRemainingCount'])) {
        // Queue up another request
    ...
    if ($Doc = $Parser->parse($errnum, $errmsg)) {
        ...
        foreach ($List->children() as $Customer) {
            $active = $Customer->getChildDataAt('CustomerRet IsActive');
            if ($active=='true'){
            $arr = array(
