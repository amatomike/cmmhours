...
public $matches;
    ...
    public function __construct(){
        ...
        $this->setup($this->loadDataFromFile());
    }
    ...
    public function loadDataFromFile(){
        $jsonfile = "assets/json/cmmdata.json";
        $dataFromFile = file_get_contents($jsonfile);
        return json_decode($dataFromFile,TRUE);
    }
    public function writeToFile($data){
        $out = file_put_contents('assets/json/cmmdata.json',$data);
     ...
     * @return mixed
     */
    public function getNames($search)
    {
        $searchArr=array();
        $added = 0;
     ...
     * @return mixed
     */
    public function getEntries($keyName)
    {
...
elseif ($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  //  echo json_encode($cmmdata,JSON_PRETTY_PRINT);
    ...
    writeToFile(json_encode($cmmdata));
    $cmmdata = loadDataFromFile();
