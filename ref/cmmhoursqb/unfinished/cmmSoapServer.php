...
$map = array(
    ...
    public function $timeTrackingResponse(){
        //ok
}
    /**
    ...
    public function addCustomerResponse($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
    {
        ...
        mysqli_query(
            "UPDATE your_customer_table SET quickbooks_listid = '" . mysqli_escape_string($idents['ListID']) . "' WHERE your_customer_ID_field = " . (int) $ID
        );
...
$obj = new cmmWorkHoursQBWC($my_example_var1, $my_example_var2);

// Map QuickBooks actions to handler functions
$map = array(
    QUICKBOOKS_IMPORT_EMPLOYEE => array( array( $obj, 'importEmployeeRequest' ), array( $obj, 'importEmployeeResponse' ) ),
    QUICKBOOKS_IMPORT_CUSTOMER => array( array( $obj, 'getCustomerRequest' ), array( $obj, 'getCustomerResponse' ) ),
    QUICKBOOKS_IMPORT_JOB => array( array($obj, 'importJobRequest'), array( $obj, 'importJobRequest')),
    QUICKBOOKS_IMPORT_SERVICEITEM => array( array( $obj, 'importServiceItemRequest' ), array( $obj, 'importServiceItemResponse' ) ),
    QUICKBOOKS_ADD_TIMETRACKING => array( array( $obj, 'addTimeTrackingRequest' ), array( $obj, 'addTimeTrackingResponse' ) )
);
...
$handler_options = array(
    ...
    'authenticate_dsn' => 'mysqli://root:easy2use@localhost/database?cmmworkhours',
...
$dsn = 'mysqli://root:easy2use@localhost/cmmworkhours';					// Connect to a MySQL database using the PHP MySQLi extension
