...
define('QB_PRIORITY_EMPLOYEE', 7);
define('QB_PRIORITY_JOB', 6);
define('QB_PRIORITY_SERVICEITEM', 5);
/**
 *
 */
...
$map = array(
    ...
    QUICKBOOKS_IMPORT_SERVICEITEM => array('_quickbooks_employee_import_request', '_quickbooks_employee_import_response'),
    QUICKBOOKS_IMPORT_EMPLOYEE => array('_quickbooks_employee_import_request', '_quickbooks_employee_import_response'),
    QUICKBOOKS_IMPORT_JOB => array('_quickbooks_job_import_request', '_quickbooks_job_import_response'),
);
...
$dsn = 'mysqli://root:easy2use@localhost/quickbooks_import';
...
if (!QuickBooks_Utilities::initialized($dsn)) {
    ...
    if (file_exists($file)) {
        ...
        foreach (explode(';', $contents) as $sql) {
            ...
            mysqli_query($GLOBALS["___mysqli_ston"], $sql) or die(trigger_error(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false))));
        }
    } else {
    ...
    if ($Doc = $Parser->parse($errnum, $errmsg)) {
        ...
        foreach ($List->children() as $Invoice) {
            ...
            foreach ($arr as $key => $value) {
                $arr[$key] = ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $value) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""));
            }
            ...
            mysqli_query($GLOBALS["___mysqli_ston"], "
				...
				)") or die(trigger_error(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false))));

            // Remove any old line items
            mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM qb_example_invoice_lineitem WHERE TxnID = '" . ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $arr['TxnID']) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")) . "' ") or die(trigger_error(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false))));
            ...
            foreach ($Invoice->children() as $Child) {
                if ($Child->name() == 'InvoiceLineRet') {
                    ...
                    foreach ($lineitem as $key => $value) {
                        $lineitem[$key] = ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $value) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""));
                    }
...
function _quickbooks_salesorder_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
{
    ...
    if ($Doc = $Parser->parse($errnum, $errmsg)) {
        ...
        foreach ($List->children() as $SalesOrder) {
            ...
            // Remove any old line items
            mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM qb_example_salesorder_lineitem WHERE TxnID = '" . ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $arr['TxnID']) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : "")) . "' ") or die(trigger_error(((is_object($GLOBALS["___mysqli_ston"])) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false))));
...
function _quickbooks_job_import_request($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){

}
function _quickbooks_job_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){
}
...
function _quickbooks_employee_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){
}
function _quickbooks_serviceitem_import_request($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){

}
function _quickbooks_serviceitem_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale){
}
/**
