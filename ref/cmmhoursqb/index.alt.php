...
public $keys;
    ...
    public function __construct(){
        ...
        $this->setup($data['Employee_List'],$data['services'],null,null,null,null);
    }
...
function checkQueryData($q,$arr){
    ...
    foreach ($arr as $v) {
        if(!is_string($v)){
            $theVals = array_values($v);
            foreach ($v as $v2) {
                if (strripos($v2, $q) !== false) {
                    array_push($searchArr, $v2);
                }
            }
        }
        else{
...
var names = new Bloodhound({
  ...
  async: true,
    prefetch: "./index.php?type=Employee_List",
  remote: {
      url: "./index.php?type=Employee_List&q=%Q",
      wildcard:"%Q"
  }
});
...
var types = new Bloodhound({
  ...
  remote: {
      url: "./index.php?type=services&q=%Q",
      wildcard:"%Q"
  }
});
