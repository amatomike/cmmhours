...
$dsn = 'mysqli://root:easy2use@localhost/quickbooks_importi';
...
$link = new mysqli("localhost", "root", "easy2use", "quickbooks_importi");
$link->connect("localhost", "root", "easy2use", "quickbooks_importi");
...
if (!QuickBooks_Utilities::initialized($dsn))
{
	...
	if (file_exists($file))
	{
		...
		foreach (explode(';', $contents) as $sql)
		{
			...
			mysqli_query($link,$sql,1) or die(trigger_error($link->error));
		}
	}
	else
	{
...
function _quickbooks_invoice_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
{
	...
	if ($Doc = $Parser->parse($errnum, $errmsg))
	{
		...
		foreach ($List->children() as $Invoice)
		{
            ...
            $mysqli = new mysqli("localhost", "root", "easy2use", "quickbooks_importi");

            foreach ($arr as $key => $value)
			{

				$arr[$key] = $link->real_escape_string($value);
			}
			...
			$link->query("
				...
				)",1) or die(trigger_error($link->error));
			
			// Remove any old line items
			$link->query("DELETE FROM qb_example_invoice_lineitem WHERE TxnID = '" . mysqli_real_escape_string($mysqli,$arr['TxnID']) . "' ",1) or die(trigger_error($link->error));
			...
			foreach ($Invoice->children() as $Child)
			{
				if ($Child->name() == 'InvoiceLineRet')
				{
					...
					{
						$lineitem[$key] = $link->real_escape_string($value);
					}
		...
		foreach ($List->children() as $Customer)
		{
			...
			$link->query("
				...
				(
					...
					'" . implode("', '", array_values($arr)) . "'
				)",1) or die(trigger_error(mysqli_error($link)));
		}
	}
	//$link->close(); 
	return true;
...
function _quickbooks_salesorder_import_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
{
	...
	if ($Doc = $Parser->parse($errnum, $errmsg))
	{
		...
		foreach ($List->children() as $SalesOrder)
		{
			...
			// Remove any old line items
			$link->query("DELETE FROM qb_example_salesorder_lineitem WHERE TxnID = '" . $link->real_escape_string($arr['TxnID']) . "' ") or die(trigger_error($link->error));
		...
		foreach ($List->children() as $Item)
		{
			...
			$link->query("
				...
				(
					...
					'" . implode("', '", array_values($arr)) . "'
				)") or die(trigger_error($link->error));
		}
	}
	//$link->close(); 
	return true;
